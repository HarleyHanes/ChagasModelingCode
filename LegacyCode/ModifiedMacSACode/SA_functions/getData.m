function [param_data,quant_data] = getData(POIs,evalFcn_,varargin)
% eg.
% getData([.1,.2],[0,1; 0.01,.1], @(x) x(1,:)*x(2,:)

%% Input Management
baseParams_=POIs.params;
baseRanges_=POIs.ranges;
...
baseQuants = evalFcn(baseParams,POIs.ref);
